...
class Classifier():
   ...
   def _clean_data(self):
      # Removes punctuation
      self.training_set[BODY_COLUMN] = self.training_set[BODY_COLUMN].str.replace('\W', ' ')

      # Removes multiple spaces with one.
      self.training_set[BODY_COLUMN] = self.training_set[BODY_COLUMN].str.replace('\s+', ' ')
   ...
   def _build_vocabulary(self):
      ...
      self.vocabulary = []
      for sms in self.training_set[BODY_COLUMN]:
         for word in sms:
   ...
   def _count_tokens(self):
      word_counts_per_sms = {unique_word: [0] * len(self.training_set[BODY_COLUMN]) for unique_word in self.vocabulary}

      for index, sms in enumerate(self.training_set[BODY_COLUMN]):
         for word in sms:
